...
type ClusterResponse = { cluster_resource: ClusterResource };
type TaskPresetItem = {
  task_type: string;
  category: string;
  updated_at: string;
  resources: { size: string; cpu: number; memory: number; gpu: number }[];
};
type RawPolicyItem = {
  task_type: string;
  size: string;
...
const ResourceManagement: React.FC = () => {
  ...
  const { data: clusterData, isLoading: loadingCluster } =
    useQuery<ClusterResponse>({
      queryKey: ["cluster_resource"],
      queryFn: () => get("/resources/cluster?node_type=task"),
      staleTime: Infinity,
    });
  const { data: quotaData, isLoading: loadingQuota } =
    useQuery<ResourceQuotaResponse>({
      queryKey: ["resource_quota"],
      queryFn: () => get("/resources/task_quota"),
      staleTime: Infinity,
    });
  const { data: taskPresetData = [], isLoading: loadingPolicy } = useQuery<
    RawPolicyItem[]
  >({
    queryKey: ["taskPreset"],
    queryFn: () =>
  ...
  const taskData = useMemo(() => {
    ...
    const grouped: Record<
      string,
      { cpu: number; memory: number; gpu: number }
    > = {};
    ...
    return Object.entries(grouped).map(([task, usage]) => ({ task, ...usage }));
  }, [quotaMap]);
  const shouldDisplay = (task: {
    ...
    const hasQuota =
      quotaMap.quota.find((q) => q.task_type === task.task)?.quota ?? 0;
  ...
  return (
    <Box>
      ...
      <Tabs value={tabIndex} onChange={(_, v) => setTabIndex(v)} sx={{ mb: 3 }}>
        {" "}
        <Tab label="자원현황" /> <Tab label="업무별 프리셋 관리" />{" "}
      </Tabs>{" "}
      {tabIndex === 0 && (
        <>
          {isLoading ? (
                ...
                <Box width="50%" pl={3}>
                  ...
                  >
                    ...
                    <Box display="flex" gap={2}>
                      {" "}
                      {(
                        Object.keys(RESOURCE_COLORS) as Array<
                          keyof typeof RESOURCE_COLORS
                        >
                      ).map((type) => (
                        <Box
                          key={type}
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>CPU</TableCell> <TableCell>Memory</TableCell>{" "}
                ...
                <TableBody>
                  {groupedTaskPresetData.map((item, index) =>
                    item.resources.map((res, resIndex) => (
                      <TableRow
                        key={`${item.task_type}-${res.size}`}
                        onClick={() =>
                          navigate(`/admin/resource/presets/${item.task_type}`)
                        }
                      ...
                      >
                        {resIndex === 0 && (
                          <>
                            {" "}
                            <TableCell rowSpan={item.resources.length}>
                              {index + 1}
                            </TableCell>{" "}
                            <TableCell rowSpan={item.resources.length}>
                              {item.task_type}
