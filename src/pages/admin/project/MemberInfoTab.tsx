...
const MemberInfoTab = () => {
    ...
    useEffect(() => {
        const fetchRoleAndUsers = async () => {
            try {
                const { data: roles = [] } = await get<{ data: any[] }>(`/projects/${CLIENT_ID}/roles`);
        ...
        <Box mt={2}>
            ...
            {members.map((user, idx) => (
                <Box
                    ...
                    borderColor="#eee"
                >
                    <Typography><b>이름:</b> {user.first_name}{user.last_name}</Typography>
                    <Typography><b>이메일:</b> {user.email ?? '-'}</Typography>
                    <Typography><b>소속:</b> {user.department ?? '-'}</Typography>
                </Box>
            ))}
        </Box>
    );
