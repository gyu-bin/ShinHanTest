...
import {
    ...
    TableHead, TableRow, Paper, CircularProgress, IconButton, Menu, MenuItem
} from '@mui/material';
import { useNavigate } from 'react-router-dom';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { useApi } from '../../hooks/useApi';
import SafetyFilterCreateDrawer from "./userDetail/SafetyFilterCreateDrawer";
...
const SafetyFilterManagement: React.FC = () => {
    ...
    return (
        <Box p={3}>
            <Typography variant="h5" fontWeight={700}>세이프티 필터 관리</Typography>
                            ...
                            <TableBody>
                                {data?.map((filter) => (
                                    <TableRow
                                        ...
                                        hover
                                        sx={{ cursor: 'pointer' }}
                                        onClick={() => navigate(`/admin/safety-filters/${filter.id}`)}
                                    >
                                        ...
                                        <TableCell align="right" onClick={(e) => e.stopPropagation()}>
                                            <IconButton onClick={(e) => handleMenuClick(e, filter.id)}>
                                                <MoreVertIcon />
                                            </IconButton>
                                        </TableCell>
                                    </TableRow>
