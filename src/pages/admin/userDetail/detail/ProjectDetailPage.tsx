...
import MemberInfoTab from '../../project/MemberInfoTab';
...
const ProjectDetailPage = () => {
    ...
    return (
        <Box p={3}>
            <Typography variant="h5" fontWeight={700}>프로젝트 상세</Typography>
            <Typography mt={1}>
                <b style={{ color: '#2b60d9' }}>'{data.project?.name}'</b> 프로젝트의 정보를 확인할 수 있습니다.
            </Typography>
            ...
            {tabIndex === 0 && (
                <Box>
                    ...
                    <Box border={1} borderColor="#eee" p={2} borderRadius={2}>
                        <Typography><b>프로젝트명:</b> {data.project.name}</Typography>
                        <Typography><b>설명:</b> {data.namespace.description ?? '-'}</Typography>
                        <Typography><b>생성자:</b> {data.namespace.creator}</Typography>
                    </Box>

                    <Typography variant="subtitle2" mt={4} mb={1}>등록정보</Typography>
                    <Box border={1} borderColor="#eee" p={2} borderRadius={2}>
                        <Typography><b>생성일시:</b> {new Date(data.namespace.created_at).toLocaleString()}</Typography>
                        <Typography><b>최종수정일시:</b> {data.namespace.modified_at ? new Date(data.namespace.modified_at).toLocaleString() : '-'}</Typography>
                    </Box>

                    <Box display="flex" justifyContent="center" gap={1} mt={3}>
                        <Button variant="outlined" color="error" onClick={() => setConfirmOpen(true)}>삭제</Button>
                        <Button variant="outlined" color="primary" onClick={() => setDrawerOpen(true)}>수정</Button>
                    </Box>
                </Box>
            )}

            {tabIndex === 1 && <MemberInfoTab />}
            {tabIndex === 2 && <Typography>역할 정보 탭 (projectId: {projectId})</Typography>}
            {tabIndex === 3 && <Typography>자원 현황 탭 (projectId: {projectId})</Typography>}
