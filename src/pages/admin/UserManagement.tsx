...
const UserManagement: React.FC = () => {
  ...
  const deleteUser = useMutation({
    mutationFn: (id: string) => deleteUserApi(`/users/${id}`),
    onSuccess: () => {
  ...
  return (
      <Box>
        <Typography variant="h5" fontWeight={700} mb={1}>사용자 관리</Typography>
        <Typography color="text.secondary" mb={3} fontSize={15}>
          플랫폼을 이용하는 모든 사용자 목록을 확인하고 관리할 수 있습니다.
        </Typography>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Button variant="contained" size="small" onClick={() => navigate('/admin/users/create')}>
            신규
          </Button>
